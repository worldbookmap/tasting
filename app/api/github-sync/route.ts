...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn("GitHub sync failed in github-sync route; returning local-only success.", syncError);
      return NextResponse.json({
        ok: true,
        mode: "demo",
        message: "GitHub sync failed, but the note was still accepted locally.",
        saved: note,
      });
    }
  } catch (error) {
